"""
TRAM评价脚本 - 同时计算 Strict-F1 和 Hierarchy-Aware-F1
用于Comment 2要求④的定量证据
"""
import os
...
def check_match_strict(pred, truth):
    """严格匹配：仅允许完全相等"""
    return pred == truth

def check_match_hierarchy(pred, truth):
    """层级宽容匹配：父子匹配算对"""
...
def compute_metrics(all_preds, all_trues, match_fn):
    global_tp, global_fp, global_fn = 0, 0, 0
    for pred_set, true_set in zip(all_preds, all_trues):
        pred_list = list(pred_set)
        true_list = list(true_set)
        for p in pred_list:
            matched = any(match_fn(p, t) for t in true_list)
            if matched: global_tp += 1
            else: global_fp += 1
        for t in true_list:
            if not any(match_fn(p, t) for p in pred_list):
                global_fn += 1
...
def main():
    ...
    # Strict match
    sp, sr, sf1, stp, sfp, sfn = compute_metrics(all_preds, all_trues, check_match_strict)
    # Hierarchy-aware match
    hp, hr, hf1, htp, hfp, hfn = compute_metrics(all_preds, all_trues, check_match_hierarchy)
